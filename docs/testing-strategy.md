...
- `EmployeeApiIntegrationTest` starts the full Spring context against H2, runs Flyway, then verifies create, search/filter, status change, and history retrieval.
- Angular tests verify API query parameter construction, employee filter reset/pagination behavior, dashboard aggregate response handling, form validation, initials, and status labels.
...
Run backend tests with `mvn -f backend/pom.xml test`; run frontend tests with `npm --prefix frontend test -- --watch=false --browsers=ChromeHeadless`.
