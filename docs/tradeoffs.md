...
| Currency-aware salary bands | The seeded currencies use deliberately different nominal ranges, so the dashboard selects documented bands per stored currency and labels every bucket with that currency. No cross-currency comparison or conversion is implied. | One set of nominal thresholds or conversion to one currency | More query branches, but avoids presenting INR and USD values as directly comparable without a rate source. |
