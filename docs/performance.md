...
Dashboard endpoints use grouped JPA projections for currency, department, country, and salary-band aggregates. Employee-to-department is a lazy relation and is mapped while the service transaction is open; salary history is fetched by an indexed employee/date query.
