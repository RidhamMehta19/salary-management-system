  ...
  template: `
    ...
    @else if (summary(); as data) {
      ...
      <section class="insight-grid">
        ...
        <p-card header="Salary distribution"><p class="card-note">Base salary bands in each employee's stored currency.</p><div class="distribution">@for (item of distribution; track item.currency + item.band) { <div class="distribution-row"><div><strong>{{ item.band }}</strong><small>{{ item.currency }}</small></div><span>{{ item.employeeCount }}</span></div> } @empty { <p class="empty-copy">No salary data yet.</p> }</div></p-card>
      </section>
